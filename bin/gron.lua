#!/usr/bin/env lua
-- gron-lua CLI entry point for LuaRocks installation

local function find_module_path()
    for path in package.path:gmatch("[^;]+") do
        local base = path:gsub("%?%.lua$", "")
        if base ~= "" then
            local test_path = base .. "gron/core.lua"
...
local base = find_module_path()
if base then
    package.path = base .. "?.lua;" .. base .. "?/init.lua;" .. package.path
end

local json = require("gron.json")
local colors = require("gron.colors")
local gron = require("gron.core")
